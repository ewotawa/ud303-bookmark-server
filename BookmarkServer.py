...
import http.server
import requests
import os # add OS for port listening 
...
def CheckURI(uri, timeout=5):
    ...
    try:
        ...
        # if a status code returns, confirm that it is 200 OK
        return r.status_code == 200
    except requests.RequestException:
        # if GET request raises exception, return false
        return False
...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("A form field is missing".encode())
        ...
        if CheckURI(longuri):
            ...
            self.send_header('Content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write("I couldn't fetch '{}'.".format(longuri).encode())
...
if __name__ == '__main__':
    ...
    server_address = ('', port) # swap out 8000 for port variable
    httpd = ThreadHTTPServer(server_address, Shortener) # update for ThreadHTTPServer class.
    httpd.serve_forever()
